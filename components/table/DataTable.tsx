...
export function DataTable<TData, TValue>({
  ...
  return (
    <div className="w-full space-y-4">
      {/* {filterOptions && <DataTableToolbar table={table} />} */}
      <div className="flex items-center justify-end gap-2">
        <div className="flex-1 self-end text-sm text-gray-500">
          Total:{" "}
          <span className="text-gray-900 font-semibold">
            {table.getFilteredRowModel().rows.length} Records
          </span>
        </div>
        {forceDisabledButton && appState.user.role === "admin" && (
          <AlertDialog open={open} onOpenChange={setOpen}>
            <AlertDialogTrigger asChild>
              <button className="px-4 py-2 bg-red-600 font-semibold rounded-md text-white hover:opacity-90">
                Force Expired
              </button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="text-base font-medium">
                  This action cannot be undone. This will All student membership expired and seats
                  reset.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <button
          className={cn(
            `flex items-center px-4 py-2.5 rounded-md border border-border text-slate-600 text-sm bg-white outline-none shadow-box transition-all`,
            {
              "bg-brand text-white": isFilter,
            }
          )}
        ...
        <TableComponent>
          <TableHeader className="border-border">
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="text-slate-700 border-b border-border">
      ...
      {peginationOption && data.length > 9 && <DataTablePagination table={table} />}
    </div>
  )
}
